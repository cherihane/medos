...
#
# Secrets requis en variables d'environnement AVANT de lancer ce script
# (jamais en dur ici — voir .env.example pour la liste complète) :
#   RESEND_API_KEY, SUPABASE_SERVICE_ROLE_KEY
# Exemple :
#   set -a && source .env.deploy && set +a && bash scripts/deploy-with-token.sh sbp_...
# (.env.deploy est ignoré par git — voir .gitignore)
# ============================================================
...
if [ -z "$RESEND_API_KEY" ] || [ -z "$SUPABASE_SERVICE_ROLE_KEY" ]; then
  echo ""
  echo "Variables d'environnement manquantes : RESEND_API_KEY et/ou SUPABASE_SERVICE_ROLE_KEY."
  echo "Voir .env.example — ne jamais coller ces valeurs en dur dans ce fichier."
  echo ""
  exit 1
fi
...
supabase secrets set \
  ...
  ADMIN_EMAIL="${ADMIN_EMAIL:-contact@kelagroup.org}" \
  APP_URL="${APP_URL:-http://81.17.98.80}" \
  WEBHOOK_SECRET="${WEBHOOK_SECRET:?WEBHOOK_SECRET manquant — voir .env.example}" \
