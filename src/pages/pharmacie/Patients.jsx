        ...
        <div style={{ backgroundColor: "white", borderRadius: 14, boxShadow: "0 1px 4px rgba(0,0,0,0.06)", overflow: "hidden" }}>
          ...
          {filtered.map((p) => (
            <div key={p.id} onClick={() => setSelected(p)}
              style={{ padding: "14px 20px", display: "flex", alignItems: "center", gap: 14, cursor: "pointer", borderBottom: "1px solid #F3F4F6", backgroundColor: selected?.id === p.id ? "#EFF6FF" : "white" }}>
              <div style={{
              ...
              <div style={{ textAlign: "right" }}>
                {p.maladies.length > 0 && <div style={{ fontSize: 11, color: "#F59E0B", fontWeight: 600 }}>{p.maladies.join(", ")}</div>}
          ...
          {!selected ? (
            ...
            <>
              <div style={{ textAlign: "center", marginBottom: 20 }}>
                <div style={{ width: 64, height: 64, borderRadius: "50%", margin: "0 auto 10px", backgroundColor: selected.genre === "F" ? "#FCE7F3" : "#DBEAFE", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 24, fontWeight: 800, color: selected.genre === "F" ? "#9D174D" : "#1D4ED8" }}>
                  {selected.name.charAt(0)}
                </div>
                <h3 style={{ margin: 0, fontSize: 16, fontWeight: 800, color: "#0A1628" }}>{selected.name}</h3>
                <div style={{ fontSize: 12, color: "#9CA3AF" }}>{selected.id}</div>
              </div>
              ...
              {[
                ...
                { label: "Ordonnances", value: selected.ordonnances },
              ].map((item) => (
